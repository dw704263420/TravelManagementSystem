...
public class UserServiceImpl implements UserService {
    ...
    public boolean userSaveUser(UserInfo userInfo) throws Exception {
        ...
        String content="<a href='http://localhost:8080/springmvc_05_json_war_exploded/user/activeUser?activeCode="+activeCode+"'>点击激活【黑马旅游网】,若无法激活，请复制localhost:8080往后的网址段到浏览器中打开</a>";
        String email = userInfo.getEmail();
        String title = "亲子旅游网用户激活邮件";
        mailUtils3.send(email,content,title);
        userInfo.setPassword(BCryptPasswordEncoderUtils.emcodePassword(userInfo.getPassword()));
        userMapper.saveUser(userInfo);
    ...
    @Override
    public List<UserInfo> queryForUserName(String userName,int page,int size) throws Exception {
        PageHelper.startPage(page,size);
        List<UserInfo> userInfos = userMapper.queryForUserName(userName);
